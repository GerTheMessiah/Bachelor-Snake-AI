...
if __name__ == '__main__':
    ...
    PLAYED_GAMES, N_ITERATIONS, BOARD_SIZE, LR_ACTOR, LR_CRITIC = 10, 1000, (8, 8), 1e-3, 1.5e-3
    ray.init(num_cpus=3)
    agent = Agent(lr_actor=LR_ACTOR, lr_critic=LR_CRITIC, gamma=0.99, K_epochs=10, eps_clip=0.10, gpu=True)
    actor_scheduler = ExponentialLR(agent.policy.base_actor.optimizer, 0.9, verbose=True)
    critic_scheduler = ExponentialLR(agent.policy.base_critic.optimizer, 0.9, verbose=True)
