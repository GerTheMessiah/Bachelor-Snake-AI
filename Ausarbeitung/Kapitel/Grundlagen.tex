...
Off-Policy RL Verfahren sind in der Lage, von Daten zu lernen, welche nicht unter der momentanen Policy \fullref{subsubsec:Grundlagen_Policy} generiert wurden. Die Aktualität der Daten spielt daher keine Rolle für dieses RL Verfahren.\\
...
\end{align} 
Der letzte Teil, welcher für die Bestimmung der PPO Fehlerfunktion benötigt wird, ist der Entropy Fehler bzw. Bonus. Dabei handelt es sich um die Entropie der Policy (Wahrscheinlichkeitsverteilung \fullref{subsubsec:Grundlagen_Policy}), welche mit der Shannon Entropie bestimmt wird. Diese ist wie folgt definiert \citep{shannon}:
\begin{align}
...
Es ergibt sich damit der bereits oben erwähnte PPO Training Loss \fullref{eq:PPO_Training_Loss}, wobei
\begin{center}
	$L^\text{PPO}_{t} (\theta) = L^\text{CLIP + VF + S}_{t} (\theta) = \mathbb{\hat{E}}_{t} [L^{\text{CLIP}}_{t}(\theta) - c_{1}L^{\text{VF}}_{t} + c_{2}S[\pi_{\theta}](s_{t})]$
\end{center}
...
\begin{enumerate}
	...
	\item Berechne die Surrogate Fehler \fullref{subsubsec:Grundlagen_Surrogate_Fehlerfunktion} $surr_1$ und $surr_2$.
	\item Bestimme den PPO Fehler. \fullref{subsubsec:Grundlagen_PPO_Fehlerfunktion}
...
\end{align}
Es wird erkenntlich, dass $Q(s,a)$ dem Q-Target $y_i$ entsprechen soll $Q(s,a) \longrightarrow y_i$ \fullref{eq:Q-Value_rekursive_def}. Darum wird die Differenz zwischen beiden bestimmt und als Fehler deklariert. Dieser wird noch quadriert, damit er stets positiv ist. \fullref{eq:DQN_Loss}\\
